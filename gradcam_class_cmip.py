...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    image = image[0, :, :, 0] # 自作モデルの時はこちらを使用 
    #image = image[0, :]
    #image -= np.min(image)
...
def main():
    ...
    # 自作モデルの場合はこちらを使用する
    preprocessed_input = load_image_self(path, val_index)
...
'''
    ...
    saliency_fn = compile_saliency_function(guided_model, activation_layer='conv2d_2')
