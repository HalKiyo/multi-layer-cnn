...
def load_image(path):
    ...
    img_path = path
    # target_size must be ajusted
    img = image.load_img(img_path, target_size=(24, 72))
    x = image.img_to_array(img)
    x = np.expand_dims(x, axis=0)
    x = preprocess_input(x)
    return x

def load_image_self(path):
    img = np.load(path)['x_val'][2]
...
def register_gradient():
    ...
    if "GuidedBackProp" not in ops._gradient_registry._registry:
        ...
        def _GuidedBackProp(op, grad):
             ...
             # tf.cast(op.inputs[0] > 0., dtype) : 入力が0以上のセルは1,0以下のセルは0の行列
...
def modify_backprop(model, name):
    ...
    with g.gradient_override_map({'Relu': name}):
        ...
        # kerasのReluをtensorflowのReLUに置き換える
        ...
        new_model = cnn.build_model()
        # originalではVGG16のモデルを使う　
...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    # 引数のinput_modelの出力層の後にtarget_layerレイヤーの追加
    # modelのpredictをすると予測クラス以外の値は0になる
    x = input_model.layers[-1].output
    x = Lambda(target_layer, output_shape=target_category_loss_output_shape)(x)
    model = keras.models.Model(input_model.layers[0].input, x)

    # 予測クラス以外の値は0なのでsumをとっても予測クラスの値のみ抽出
    ...
    # 予測クラスの値から最後のconv層までの勾配を計算する関数を定義
    # 定義した関数の
    # 入力 : [判定したい画像.shape=(1, 224, 224, 3)],
    # 出力 : [最後のconv層の出力値.shape=(1, 14, 14, 512), 予測クラスの値から最後のconv層までの勾配.shape=(1, 14, 14, 512)]
    ...
    #cam = np.zeros(output.shape[0 : 2], dtype = np.float32) #自作モデルではこちらを使用
    ...
    cam = cv2.resize(cam (24, 72))
    # 負の値を0に置換、処理としてはReLUと同じ
    cam = np.maximum(cam, 0)
    # 値を0~1に正規化
    #heatmap = cam / np.max(cam) # originalではこちらを使う　
    ...
    # 自作モデル has masked value, then normalization process is not needed.
    # Just extracting 1 channel for checking the grad cam
    # if value is too small, multiply the coefficient so that cam and image has same order.
    image = image[0, :, :, :3] # 自作モデルの時はこちらを使用 
    #image = image[0, :]
    #image -= np.min(image)
    #image=np.minimun(image, 255)
...
def main():
    ...
    # 自作モデルの場合はこちらを使用する
    path = '/docker/home/hasegawa/docker-gpu/multi-layer-cnn/result_cnn_class_cmip/input/0.5575.npz'
    weights_path = '/docker/home/hasegawa/docker-gpu/multi-layer-cnn/result_cnn_class_cmip/weights/0.5575.h5'
    preprocessed_input = load_image_self(path)
    ...
    # 3. 入力画像の予測確率(predictions)と予測クラス(predicted_class)の計算
    ...
    cam, heatmap = grad_cam(model, preprocessed_input, predicted_class, "conv2d_2")

    # 5. 画像の保存
    cv2.imwrite("gradcam.jpg", cam)

    # 6. GuidedBackPropagation用勾配の実装
    register_gradient()
    # 7. ReLUの勾配計算をGuidedBackPropagationの勾配計算に変更
    guided_model = modify_backprop(model, 'GuidedBackProp')
    # 8. GaidedBackPropagation計算用の関数の定義
    # 自作クラスを使う場合は、こちらの引数の最後のconv層のレイヤー名を追加で指定
    saliency_fn = compile_saliency_function(guided_model)
    # 9. GaidedBackPropagationの計算
    saliency = saliency_fn([preprocess_input, 0])
    # 10. Guided Grad-CAMの計算
